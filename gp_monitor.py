...
from graph_tool.draw import graph_draw, interactive_window, sfdp_layout, GraphWindow, GraphWidget
from graph_tool.topology import label_components
from matplotlib.cm import viridis, plasma, inferno, magma, cividis
from matplotlib.cm import Greys as greys, Purples as purples, Blues as blues, Greens as greens, Oranges as oranges, Reds as reds
from matplotlib.cm import cool, hot, gist_heat
from matplotlib.cm import gnuplot, gnuplot2, CMRmap as cmrmap, rainbow, jet, nipy_spectral
...
class egp_GraphWidget(GraphWidget):
    ...
    def motion_notify_event(self, widget, event):
        ...
        if not (state & Gdk.ModifierType.BUTTON1_MASK and
            not state & Gdk.ModifierType.CONTROL_MASK and
...
graph = egp_GraphWidget(g, fit_view=True, pos=pos, vcmap=jet, vertex_fill_color=s,
                            bg_color=(1,1,1,1))
...
class AppWindow(Gtk.ApplicationWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.grid.attach_next_to(self.separator, self.label1, Gtk.PositionType.BOTTOM, 1, 1)
        self.grid.attach_next_to(self.label2, self.separator, Gtk.PositionType.BOTTOM, 1, 4)
    ...
    def motion_notify_event(self, widget, event):
        if isinstance(graph.picked, Vertex):
            label2_text = self.label2_text.format(refstr[graph.picked],
                '{:0.3f}'.format(s[graph.picked]))
